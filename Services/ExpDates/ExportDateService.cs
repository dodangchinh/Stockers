...
namespace Chinh_QuanLyKho
{
    ...
    {
        public ExportDateService()
        {
        }

        public void Add(ExpDate expDate)
        {
            UnitOfWork.Instance.exportDateRepository.Add(expDate);
        }
        public void AddList(List<ExpDate> lstExpDate)
        {
            foreach (var item in lstExpDate)
                UnitOfWork.Instance.exportDateRepository.Add(item);
        ...
        {
            UnitOfWork.Instance.exportDateRepository.Update(expDate);
        }
        public void UpdateList(List<ExpDate> lstExpDate)
        {
            foreach (var item in lstExpDate)
                UnitOfWork.Instance.exportDateRepository.Update(item);
        }

        public void Delete(ExpDate expDate)
        {
            UnitOfWork.Instance.exportDateRepository.Delete(expDate);
        }
        public ExpDate GetByQuantity(string IdProduct)
        {
            foreach (var item in UnitOfWork.Instance.exportDateRepository.Gets())
        ...
        {
            return UnitOfWork.Instance.exportDateRepository.Gets();
        }
    }
}
